...
class BrownfieldDatasetAnalyser(DatasetAnalyser):
    ...
    def total_hectares(self):
        hectares = [
            float(x["hectares"]) for x in self.json_data if x["hectares"] is not None
        ]
...
if __name__ == "__main__":
    da = BrownfieldDatasetAnalyser(
        "https://raw.githubusercontent.com/digital-land/brownfield-land-collection/master/index/dataset.csv"
    )
    da = BrownfieldDatasetAnalyser(
        "https://raw.githubusercontent.com/digital-land/brownfield-land-collection/master/index/entries.csv"
    )
    sites = [
        2,
        234,
